...
协议与最终主实验 run_joint.py 完全一致(可引用):
  ...
  软/硬罚 0.3/3.0)、固定纵断面首末端点、无建筑密度约束、OSM交叉桥内生;
...
结果缓存 results/sweep40_c8c_w500_nodens_fixedend.json, 已存在则直接出图。
"""
...
RES_FN = os.path.join(HERE, "results", "sweep40_c8c_w500_nodens_fixedend.json")
...
def run_sweep(workers):
    ...
    out = dict(meta=dict(n_weights=N_W, pop=200, max_iter=1000,
                         ...
                         protocol="run_ijs_two_phase soft0.3/hard3.0, "
                                  "joint_baseline seed2025+现状注入, 固定纵断面首末端点, "
                                  "无建筑密度约束, OSM交叉桥内生口径",
