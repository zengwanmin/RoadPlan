...
SOURCE_FINGERPRINT_FILES = (
    "run_joint.py", "fair_pareto.py", "objective_joint.py", "objective.py", "algorithms.py",
...
def main():
    ...
    # 尺度。不再产生或保留单个固定熵权解；最终熵权必须等两条
    # Pareto 前沿都完成后，在两者合并的共同范围上计算。
    x_A = make_existing_x(pc, dim)
    base, _, _, C_ref, E_ref = joint_baseline(pc, POP_SIZE, x_seed=x_A)
    print(f"[参考尺度] C_ref={C_ref/1e8:.4f}亿, E_ref={E_ref/1e8:.4f}亿")

    pop0 = base.copy()          # 全部Pareto权重点共享初始种群保证公平

    config = dict(
        schema="joint-pareto-resume-v3",
        ...
        weight_grid=np.linspace(0.0, 1.0, n_pareto).tolist(),
    ...
    # ---------- 并行求解联合方法 Pareto 权重扫描 ----------
    # 全部权重点使用同一 pop0 和 seed=1000，不再额外运行固定熵权
    # M-C。wC=1 的 Pareto 端点同时作为 M-B，避免重复评价。
    w_grid = np.linspace(0.0, 1.0, n_pareto)
    tasks = [dict(tag=f"pareto_{k}", wC=float(w), wE=float(1 - w), seed=1000)
    ...
    # ---------- Pareto 权重扫描结果整理(含 best_x/curve/nfe) ----------
    pareto_sweep = []
    for k, w1 in enumerate(w_grid):
        rec = solved[f"pareto_{k}"]
        pareto_sweep.append(dict(tag=f"pareto_{k}", w1=float(w1),
                                 C=rec["C"], E=rec["E"], pen=rec["pen"],
                                 best_x=rec["best_x"], curve=rec["curve"],
                                 nfe=int(rec["nfe"])))

    # wC=1 是 Pareto 网格端点，与 M-B 定义完全一致。
    rB = solved[f"pareto_{n_pareto - 1}"]
    res_B = evaluate_joint(np.array(rB["best_x"]), pc)
    print(f"[M-B/Pareto端点] C={res_B['C']/1e8:.4f}亿 "
          f"E={res_B['E']/1e8:.4f}亿 L={res_B['L_km']:.3f}km "
          f"Rmin={res_B['Rmin']:.0f}m pen={res_B['penalty']:.2e}")

    print("[共同决策] 联合原始前沿已完成；最终 M-C 将由 "
          "run_twostage.py 在两条前沿合并后用公共熵权选出。")
    ...
    out = dict(
        ...
        decision_status="pending_common_decision_with_two_stage_front",
        measured=dict(x=align["X"].tolist(), y=align["Y"].tolist()),
        convergence_B=rB["curve"],
    )
