...
用法:
  python3 run_ablation.py            # 正式全量 (pop200/iter300/10次)
  python3 run_ablation.py --smoke    # 冒烟测试 (iter=5, 3次, 验证管线)

【为何本实验串行执行】表A2 把"平均运行时间(s)"作为结果列上报, 若跨进程并行,
各变体的耗时会受内存带宽/缓存争用干扰而不再可比。因此坚持单进程串行,
保证 8 个变体的运行时间在同一条件下测得、可直接横向比较。
"""
...
def main():
    ...
    max_iter = ALGO["max_iter"]      # 300
    n_runs = ALGO["n_runs"]          # 10
    if args.smoke:
        max_iter, n_runs = 5, 3
    ...
    def ce(x):
        return np.asarray(objectives(x, ctx)[:2], dtype=np.float64)

    CE0 = evaluate_many_ordered(ce, base_pop)
    C0, E0 = CE0[:, 0], CE0[:, 1]
    ...
    out = dict(
        meta=dict(pop_size=pop_size, max_iter=max_iter, n_runs=n_runs,
                  ...
                  tent_initialization_replaces_pop0=True,
