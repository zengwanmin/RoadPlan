...
def table_A3(d, td):
    ...
    for k in _order(d):
        ...
        init_kind = "Tent替代" if has_tent else "随机"
        tent_gain = None
        base_key = tent_pair_base.get(k)
        if base_key in variants:
            base_fs = np.asarray(variants[base_key]["best_fs"], dtype=float)
            tent_fs = np.asarray(variants[k]["best_fs"], dtype=float)
            if len(base_fs) == len(tent_fs):
                valid = (np.isfinite(base_fs) & np.isfinite(tent_fs)
                         & (np.abs(base_fs) > 1e-12))
                if np.any(valid):
                    tent_gain = float(np.median(
                        100.0 * (base_fs[valid] - tent_fs[valid])
                        / base_fs[valid]))
        ...
        rows.append((LABEL[k], init_kind, fmt(tent_gain, 2),
...
def fig_A4(d, td):
    ...
    v = d["variants"]
    required = ["V1_JS", "V2_JS+Tent", "V3_JS+Levy", "V4_JS+DE"]
    ...
    paired_specs = [
        ("V2_JS+Tent", "+Tent (V2 vs V1)", "#1f77b4"),
