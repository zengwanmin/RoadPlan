...
# =====================================================================
# 四、选定版本(方案A = V1)的分区产物导出
# =====================================================================
V1_PARAMS = dict(metric="area", sigma=200.0, margin=1.15,
                 close_m=100.0, min_area_ha=1.0, pass_ratio=0.5)


def export_tiers(npz_path, lat0_deg, lon0_deg, route_xy, out,
                 params=None, grids_cache=None, verbose=True):
    """
    按选定版本参数导出分区产物, 供优化器直接加载。

    depth2(禁区内深度, m)是关键: 惩罚项若直接用布尔掩膜, 在禁区**内部**是平台状的、
    梯度恒为 0, IJS 在里面感受不到任何方向, 无法被"推"出禁区; 而距离变换给出的深度场
    处处指向最近的禁区边界, 提供明确的逃离方向。这与 dem.py 中"高程用 _bilinear 保连续、
    掩膜用最近邻"的既有取舍是同一考虑。
    """
    p = dict(V1_PARAMS)
    if params:
        p.update(params)
    G = rasterize(npz_path, lat0_deg, lon0_deg, cache=grids_cache, verbose=verbose)
    S = density_field(G[p["metric"]], p["metric"], p["sigma"])
    rx, ry = route_xy
    D_A_max, th_f, th_p, D_A = calibrate(S, rx, ry, p["margin"], p["pass_ratio"])
    tier2, tier1, nclu = cluster_tiers(S, th_p, th_f, p["close_m"], p["min_area_ha"])

    # 禁区内深度(m): 距离变换以像元为单位, 乘 CELL 换算成米
    depth2 = ndimage.distance_transform_edt(tier2) * CELL

    np.savez_compressed(
        out,
        S=S.astype(np.float32),
        tier1=tier1.astype(np.uint8),
        tier2=tier2.astype(np.uint8),
        depth2=depth2.astype(np.float32),
        theta_pass=th_p, theta_forbid=th_f, D_A_max=D_A_max,
        metric=p["metric"], sigma=p["sigma"], margin=p["margin"],
        close_m=p["close_m"], min_area_ha=p["min_area_ha"],
        n_clusters=nclu,
        X0=X0, X1=X1, Y0=Y0, Y1=Y1, CELL=CELL, NX=NX, NY=NY,
    )
    if verbose:
        cell_km2 = CELL ** 2 / 1e6
        print(f"[导出] {out}")
        print(f"  指标={p['metric']} σ={p['sigma']:.0f}m margin={p['margin']:.2f} "
              f"闭运算={p['close_m']:.0f}m 最小簇={p['min_area_ha']:.1f}ha")
        print(f"  D_A_max={D_A_max:.4f}  θ_pass={th_p:.4f}  θ_forbid={th_f:.4f}")
        print(f"  Tier2 {tier2.sum()*cell_km2:.2f} km² / {nclu} 簇   "
              f"Tier1 {tier1.sum()*cell_km2:.2f} km²")
        print(f"  禁区最大深度 {depth2.max():.0f} m")
    return dict(S=S, tier1=tier1, tier2=tier2, depth2=depth2,
                theta_pass=th_p, theta_forbid=th_f, D_A_max=D_A_max,
                n_clusters=nclu)
