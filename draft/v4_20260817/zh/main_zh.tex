...
\begin{abstract}
高速公路线形同时耦合平面几何、纵断面、地形、桥隧、城市建设强度与长期车辆运行。本文构建数据驱动双目标框架，将准天然数字高程模型（DEM）、OpenStreetMap（OSM）交通/水系/建筑图层和14,018个GNSS点融合至统一数字走廊，以50个低频平面模态和224个被读取坡度基因协同描述三维线形；均值中心化后坡度局部独立自由度至多为223。全生命周期成本包含占地、基本工程、结构、土方和养护；第二目标将30年油电混行车流的燃油与电耗货币化。候选线形与OSM障碍物相交时触发跨越桥，连通山体触发生态隧道，高密度建成区核心禁行。IJS用于加权和前沿近似，所得候选依次通过硬可行、预算、非支配与熵权折中筛选。
...
\item 构建名义275槽、274个被读取基因、至多273个局部独立自由度的平纵协同参数化，并实现成本—油电混行能耗双目标决策；
...
NSGA-II是常用多目标基准\cite{deb2002}，但高维纵断面变量会显著增加进化搜索负担。JS模拟水母随洋流运动及主动/被动运动\cite{chou2021}。本文在JS上增加Tent、L\'evy和DE\cite{storn1997}；已有消融仅报告旧模型同代数下的探索性关联，各组件效应仍需在当前模型上等NFE复核。熵权法以Shannon信息量为基础\cite{shannon1948}；本文先进行硬可行、预算和非支配筛选，再在剩余前沿中定权选点，从程序上禁止“权重抵消违约”。
...
\label{eq:grade_zh}
\end{align}
其中$\lambda$取保证$|g_i|\leq g_{\max}$的最大缩放值。因此$\sum_i g_i\Delta s_i=z_L-z_0$恒成立，优化器无法通过整体下倾获取虚假节能。生成权威案例线位的实现仍保留225槽纵断面块，其中端点锚定时起点高程槽被忽略，故存储向量为275槽，代码实际读取274个基因（50个模态和224个坡段）。由于224个原始坡度同时增加同一常数会被均值中心化消除，局部独立设计自由度至多为273（50+223）；约150个平面重建控制点不属于决策变量。

确认性实验在不改变解码器的前提下消除上述两个冗余方向。对原始坡度基因$r\in[0,1]^{224}$，定义商空间坐标
\begin{equation}
q_i=r_i-r_{224}+\tfrac12,\quad i=1,\ldots,223,
\qquad q_i\in[-\tfrac12,\tfrac32],
\end{equation}
并重构$r'=(q_1,\ldots,q_{223},1/2)$。由于$r'$仅相对$r$作整体平移，扣除里程加权均值后的中心化坡度向量完全相同；未读取的起点槽固定为$1/2$。因此确认性搜索包含50个平面模态和223个坡度对比坐标，共273个非冗余变量，同时保持端点锚定、坡度缩放、DEM/OSM查询与工程目标不变。
...
\end{align}
$h_j$的正负分别选择填方—桥梁或挖方—隧道费率组合；结构豁免桩号取$c_j=0$。当填/挖单价超过桥/隧单价，或高填深挖超过30 m时触发结构替代；结构区土方豁免，从而避免重复计价和“悬浮免费”退化解。
...
\begin{equation}
e_e=\left[\frac{1}{3.6\times10^6}\sum_i
\begin{cases}W_i/\eta_e,&W_i\geq0,\\W_i\eta_e,&W_i<0,\end{cases}
\right]_+ +0.15L_{km},
\end{equation}
其中$[x]_+=\max(x,0)$，$\eta_e=0.90\times0.95$。当前主实验计算代表方向，端点锚定消除了净落差套利；双向平均仍待完善。
...
因此IJS每代约包含3倍种群评价量，本文同时报告NFE/时间，避免把同代数误认为同预算。权威单次案例采用前半程0.3倍、后半程3倍的两阶段惩罚调度；下述等NFE确认性比较则对全部算法始终采用同一个静态强罚目标。图\ref{fig:flow_zh}给出原案例求解闭环。
...
确认性实验冻结当前端点锚定、DEM/OSM与密度启用模型，并固定权威案例结果中的熵权$(w_C,w_E)=(0.50838,0.49162)$及参考尺度。实验采用非冗余273维表示、种群200，每次运行严格限制为600,000次目标调用；Tent、L\'evy、DE及所有基线试探均计入NFE，运行后诊断既不计入预算，也不向搜索器反馈。20个预注册重复在全部算法间共享持久化初始种群及SHA-256；首个个体为可行M-A，其余个体先在原275槽定义域采样，再映射到商空间坐标。算法随机种子按重复编号配对。

所有确认性求解器最小化同一个静态目标
\begin{equation}
F=0.50838\,C/C_{ref}+0.49162\,E/E_{ref}+3P+0.22L_{T1}/L.
\end{equation}
与历史主搜索不同，确认性比较不切换软/硬惩罚，从而避免用软阶段旧适应度与硬阶段新适应度直接比较，并保证全部算法采用相同可行性处理。算法比较包括JS、GA、PSO、GWO和NSGA-II；IJS机制消融采用Tent--L\'evy--DE完整$2^3$因子设计。GA、PSO和GWO逐式保留仓库基线及参数，仅增加精确NFE截止。NSGA-II在两个带罚归一化目标上搜索，再从全部已评价非支配候选中按同一冻结加权和选点。

主要配对比较对$F$采用双侧Wilcoxon符号秩检验，并在各预注册比较族内作Holm校正；同时报告配对秩二列效应量、配对中位差、20,000次配对bootstrap 95\%区间、精确配对可行性检验、原始样本和运行时间，$C/E$作为次要结局单列。当前模型结构消融比较联合、仅平面、仅纵断面与先平后纵（300,000+300,000 NFE）。稳健性重优化以10个配对种子考察$W=250/1000$ m、关闭密度层、原始/道路重建DEM、OSM桥梁延伸50/100 m及强制结构阈值20/40 m。车队、需求与价格情景属于优化后评价，不冒充重新优化。
...
对应的观察差距图和运行分布图保留在本版本的\texttt{figures}目录中，作为补充审计材料，不在主文中包装为当前模型的确认性证据。

历史机制插桩显示，独立Tent链替换了200个初始个体中的132个；使用DE与末100代约0.018的较低$F$相关，这与后期精修假设一致，但不能排除额外求值预算的影响。完整IJS几乎没有持续20代停滞，因此预设“停滞后逃逸率”指标不适用，不能将其机械记为0并解释为L\'evy无效。
...
六规模收敛图和PJ6分布图保留在本版本的\texttt{figures}目录中，作为补充审计材料。因NFE不等，主文不再用视觉排名强化算法优越性。
...
历史谱宽参数表现出先快速、后饱和的降本规律：匹配重优化实验中，$W=200$ m时$C=25.68$亿元，$W=2{,}500$ m时降至22.39亿元；谱搜索具备绕离生态隧道区的空间后，边际收益逐步受城市与曲率约束限制。桥梁两端延伸$E_{ext}=50/75/100$ m时，$C$分别为22.81/23.55/24.74亿元，而$E$保持约13.55亿元。由于该实验来自旧自由端点模型，它仅用于提出假设，不能证明当前模型稳健性。

\begin{figure*}[t]
\centering
\begin{subfigure}{.78\textwidth}\includegraphics[width=\linewidth]{sensitivity_tornado.pdf}\caption{因素重要性。}\end{subfigure}\\[3mm]
\begin{subfigure}{.78\textwidth}\includegraphics[width=\linewidth]{corridor_sensitivity.pdf}\caption{历史谱宽重优化。}\end{subfigure}
\caption{历史自由端点敏感性；每个点均重优化，但实验早于端点锚定与密度分区；$W$不是几何硬边界。}
\end{figure*}
\FloatBarrier
...
因此本文结论仅支持可审计的单次走廊筛选与后续假设生成，尚不支持确认性方法比较，也不能直接替代详细路线和施工图设计。
...
\section*{数据与代码可用性}
论文由RoadPlan实验仓库生成。GNSS及项目派生文件可能受数据所有者限制；处理后的DEM/OSM、源代码、固定随机种子、结果JSON和制图脚本已按复现逻辑组织。本地交付包另含\texttt{SUPPLEMENT\_REPRODUCIBILITY.md}、源值快照与SHA-256清单。双盲审稿后应补充公开仓库与DOI（当前为数据仓库DOI占位符）。
