...
The immutable JSON's free-text `meta.note` retains two stale labels (7.8-km constant bridge accounting and a hard ±500-m corridor). Numerical fields are unchanged and authoritative; `IMPLEMENTATION_METADATA_SIDECAR.json` records the current OSM-triggered bridge interpretation, spectral definition of `W`, and the Tent-map documentation erratum.
...
- The stored current vector has 275 slots. Its reserved start-elevation gene is ignored, 274 genes are referenced, and grade centering leaves at most 273 locally independent degrees of freedom.
...
- Headline C, E and Tier-1 percentages, zero penalty and zero Tier-2 exposure are executable assertions against the raw JSON.
- `tables/source_value_snapshot.csv` contains 210 machine-extracted source values with evidence-scope labels and source hashes. It is a source snapshot, not a parser-based audit of every number printed in LaTeX.
- `tables/source_manifest.csv` includes the current result, historical evidence tables, current objective/algorithm/geometry code, DEM, OSM obstacles/buildings, density tiers and the primary alignment workbook.
- The maximum sampled M-C lateral offset (approximately 184 m) is documented as a derived geometry diagnostic; it is not yet among the five executable headline assertions.
