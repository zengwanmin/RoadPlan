...
def decode_joint(x, pc, design_z_ctrl=None):
    """
    ...
    if design_z_ctrl is None:
        design_z_ctrl = profile_from_grades(x[N_MODE:], gz_ctrl, ds_ctrl,
                                           LONG_STD_100["grade_max"], z_tie=z_tie)
    else:
        # 双层求解: 纵断面由 dp_profile 全局最优解直接给出, 不经纵坡编码往返
        # (端点锚定含均坡扣除, 编码往返不是精确逆变换, 会引入失真)
        design_z_ctrl = np.asarray(design_z_ctrl, float)
        if len(design_z_ctrl) != len(sta_ctrl):
            raise ValueError(
                f"design_z_ctrl 长度 {len(design_z_ctrl)} != 变坡点数 {len(sta_ctrl)}")
...
def objectives_joint(x, pc, pen_scale=1.0, scenario=None, design_z_ctrl=None):
    ...
    d = decode_joint(x, pc, design_z_ctrl=design_z_ctrl)
